...
- **Junior coder** (`junior-coder`) — single-file or single-module change, no external impact, no dependency wrangling, no security-sensitive code, linear logic.
- **Senior coder** (`senior-coder`) — anything else: cross-module work, dependency changes, refactors with public-API impact, security-touching code, async/concurrency, performance-sensitive paths.
...
Dispatch:
- Single task: `{ agent: "senior-coder", task: "<prompt above>" }` or `junior-coder`.
...
Dispatch with `{ agent: "reviewer", task: "<prompt above>" }`.
